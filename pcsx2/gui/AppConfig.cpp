...
#define GAMEINDEX_DIR_str(s) #s
	...
	wxDirName GetScripts()
	{
        return GetDocuments() + Base::Scripts();
	}
...
#define PLUGIN_DIR_str(s) #s
	...
	wxDirName Get( FoldersEnum_t folderidx )
	{
		switch( folderidx )
		{
			...
			case FolderId_Scripts:		return GetScripts();
...
wxDirName& AppConfig::FolderOptions::operator[]( FoldersEnum_t folderidx )
{
	switch( folderidx )
	{
        ...
        case FolderId_Scripts:		return Scripts;
...
bool AppConfig::FolderOptions::IsDefault( FoldersEnum_t folderidx ) const
{
	switch( folderidx )
	{
        ...
        case FolderId_Scripts:		return UseDefaultScripts;
...
void AppConfig::FolderOptions::Set( FoldersEnum_t folderidx, const wxString& src, bool useDefault )
{
	switch( folderidx )
	{
		...
		case FolderId_Scripts:
			Scripts = src;
            UseDefaultScripts = useDefault;
		break;
...
wxDirName GetScriptsFolder()
{
    return GetResolvedFolder(FolderId_Scripts);
}
...
void AppConfig::FolderOptions::ApplyDefaults()
{
    ...
    if( UseDefaultScripts )		Scripts		  = PathDefs::GetScripts();
}
